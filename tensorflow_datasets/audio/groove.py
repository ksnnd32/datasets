...
_DESCRIPTION = """\
The Groove MIDI Dataset (GMD) is composed of 13.5 hours of aligned MIDI and
...
class GrooveConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, split_bars=None, include_audio=True, audio_rate=16000,
    ...
    """
    name_parts = [("%dbars" % split_bars) if split_bars else "full"]
    if include_audio:
...
class Groove(tfds.core.GeneratorBasedBuilder):
  ...
  def __init__(self, split_bars=None, include_audio=True, audio_rate=16000,
    ...
    if not config:
      config = GrooveConfig(
          split_bars, include_audio, audio_rate, version="0.0.0")
    super(Groove, self).__init__(config=config, **kwargs)
  ...
  def _info(self):
    ...
    if self.builder_config.include_audio:
      features_dict["audio"] = tfds.features.Sequence(tf.int32)
    ...
    for row in rows:
      ...
      if self._builder_config.include_audio:
        if not row["audio_filename"]:
          ...
          continue
        with tf.gfile.Open(
            os.path.join(data_dir, row["audio_filename"]), "rb") as audio_f:
          audio_segment = tfds.core.lazy_imports.pydub.AudioSegment.from_file(
              audio_f, format="wav").set_channels(1).set_frame_rate(
                  self._builder_config.audio_rate)
          audio = np.array(
              audio_segment.get_array_of_samples()).astype(np.int32)
      ...
      if not split_bars:
        ...
        pm = tfds.core.lazy_imports.pretty_midi.PrettyMIDI(StringIO(midi))
